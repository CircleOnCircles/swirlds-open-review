...
public class SyncShadowGraphDFSIterator implements Iterator<SyncShadowEvent> {
	...
	void pushNext() {
		if (!visited.contains(cur.getOtherParent()) && !st.contains(cur.getOtherParent()) && cur.getOtherParent() != null) {
			st.push(cur.getOtherParent());
		}

		if (!visited.contains(cur.getSelfParent()) && !st.contains(cur.getSelfParent()) && cur.getSelfParent() != null) {
			st.push(cur.getSelfParent());
		}

		cur.getOtherChildren().forEach((SyncShadowEvent s) -> {
			if (!visited.contains(s)) {
				st.push(s);
			}
		});

		cur.getSelfChildren().forEach((SyncShadowEvent s) -> {
			if (!visited.contains(s)) {
				st.push(s);
			}
		});
