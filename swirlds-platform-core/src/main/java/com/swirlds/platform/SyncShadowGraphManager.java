...
//					inserted.add(e);
	...
	enum VerificationStatus {
		PASS(0),
		NOT_IN_SHADOW_GRAPH(1),
		MISMATCHED_EVENT_HASH(2),
		MISSING_SELF_PARENT(3),
		MISSING_OTHER_PARENT(4),
		CONTAINS_EXPIRED_SELF_PARENT(5),
		CONTAINS_EXPIRED_OTHER_PARENT(6),
		SELF_PARENT_MISMATCH(7),
		OTHER_PARENT_MISMATCH(8),
		MISSING_INBOUND_SELF_CHILD_POINTER(9),
		MISSING_INBOUND_OTHER_CHILD_POINTER(10),
		MISSING_INBOUND_SELF_PARENT_POINTER(11),
		MISSING_INBOUND_OTHER_PARENT_POINTER(12),
		CONTAINS_INVALID_TIP(13),
		MISSING_TIP(14),
		INVALID_STATUS(15);

		VerificationStatus(final int value) {
			this(value, "");
		}

		VerificationStatus(final int value, final String msg) {
			this.value = value;
			this.msg = msg;
		}

		static VerificationStatus getStatus(final int value) {
	...
	synchronized VerificationStatus verify(Hashgraph hashgraph) {
		...
		if (events.length == 0) {
			return VerificationStatus.getStatus(0);
		}
		...
		for (EventImpl e : events) {
			...
			if (e.getSelfParent() != null) {
				if (s.selfParent == null) {
					...
					return VerificationStatus.getStatus(3);
				}
			}
			...
			if (e.getOtherParent() != null) {
				if (s.otherParent == null) {
					...
					// was constructed may contain expired events (with expiration as defined by the
					// hashgraph object), but this return value should be transitory in the call sequence.
					return VerificationStatus.getStatus(4);
				}
			}
			...
			if (e.getSelfParent() == null) {
				if (s.selfParent != null) {
					...
					return VerificationStatus.getStatus(5);
				}
			}
		...
		for (EventImpl e : events) {
			...
			if (isSelfParent && tips.contains(shadow(e))) {
				return VerificationStatus.getStatus(13);
			}

			// Internal failure: tip identification. Should never happen.
			if (!isSelfParent && !tips.contains(shadow(e))) {
				return VerificationStatus.getStatus(14);
			}
		}
	...
	synchronized void resetForSync() {
		workingTips.clear();
	...
	synchronized void setReceivedTipHashes(List<Hash> receivedTipHashes) {
		this.sendList.clear();
	...
	synchronized List<Boolean> getSendTipFlags() {
	...
	synchronized void setReceivedTipFlags(List<Boolean> receivedTipFlags) {
	...
	synchronized List<EventImpl> getSendEventList(NodeId selfId, NodeId otherId) {
		log.debug(SYNC_SGM.getMarker(), "{} -> {} `getSendEventList`: finishing sendList, starting with {} events...",
				selfId, otherId, sendList.size());
		List<EventImpl> sendList = getSendEventList();
		log.debug(SYNC_SGM.getMarker(), "{} -> {} `getSendEventList`: ...done. sendList has {} events", selfId, otherId,
				sendList.size());
		return sendList;
	}

	synchronized List<EventImpl> getSendEventList() {
...
//			System.out.println(" receivedEvent = " + receivedEvent.getBaseHash().toString().substring(0, 4) + " seq.
//			number = " + receivedEvent.getSeq());
	...
	private InsertableStatus insertable(Event e) {
		...
		if (expired(e)) {
//			System.out.println(((EventImpl) e).getBaseEventHashedData().getHash().toString().substring(0, 4) + ": not
//			insertable (expired)");
...
//				System.out.println(((EventImpl) e).getBaseEventHashedData().getHash().toString().substring(0, 4) + ":
//				not insertable (other parent unexpired and unknown)");
...
//				System.out.println(((EventImpl) e).getBaseEventHashedData().getHash().toString().substring(0, 4) + ":
//				not insertable (self parent unexpired and unknown)");
