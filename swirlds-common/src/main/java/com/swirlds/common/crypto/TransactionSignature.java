...
public class TransactionSignature implements Comparable<TransactionSignature> {
	 ...
	 * 		array
	 */
	public TransactionSignature(final TransactionSignature other, final int publicKeyOffset, final int publicKeyLength,
			final int messageOffset, final int messageLength) {
	...
	private static void serialize(final TransactionSignature signature, final DataOutputStream dos,
		...
		// Write Public Key
		dos.write((signature.expandedPublicKey != null) ? signature.expandedPublicKey : signature.contents,
				signature.publicKeyOffset, signature.publicKeyLength);
		...
		if (byteCount != null && byteCount.length > 0) {
			byteCount[0] += (7 * Integer.BYTES) + (signature.signatureLength * Byte.BYTES)
					+ (signature.publicKeyLength * Byte.BYTES) + (signature.messageLength * Byte.BYTES);
		}
	}
	...
	public static TransactionSignature deserialize(final DataInputStream dis, final int[] byteCount) throws IOException {
		...
		if (sigLen < 0 || sigChecksum != (439 - sigLen)) {
			throw new BadIOException("Signature.deserialize tried to create signature array of length "
		...
		if (pkLen < 0 || pkChecksum != (541 - pkLen)) {
			throw new BadIOException("Signature.deserialize tried to create public key array of length "
	...
	public void computeDigest(final MessageDigest digest) {
		...
		HashUtils.update(digest, (expandedPublicKey != null) ? expandedPublicKey : contents, publicKeyOffset,
				publicKeyLength);
	...
	public boolean equals(final Object obj) {
		...
		return messageOffset == signature.messageOffset && messageLength == signature.messageLength
				&& publicKeyOffset == signature.publicKeyOffset && publicKeyLength == signature.publicKeyLength
				&& signatureOffset == signature.signatureOffset && signatureLength == signature.signatureLength
				&& Arrays.equals(contents, signature.contents) && signatureType == signature.signatureType;
	}
	...
	@Override
	public int hashCode() {
		int result = Objects.hash(messageOffset, messageLength, publicKeyOffset, publicKeyLength, signatureOffset,
